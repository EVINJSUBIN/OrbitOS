...
const WALLPAPERS = [
  { id: 'wall-0', src: mediaUrl('/images/wallpapers/wall-0.jpg') },
...
export default function Settings() {
  const [accent, setAccent] = useState(() => localStorage.getItem('OrbitOS-accent') || 'sky')
  const [bgMode, setBgMode] = useState<'dark' | 'static' | 'live'>(() => (localStorage.getItem('OrbitOS-bg') as any) || 'static')
  const [selectedWallpaper, setSelectedWallpaper] = useState(() => localStorage.getItem('OrbitOS-wallpaper') || 'wall-0')
  const [selectedLiveWall, setSelectedLiveWall] = useState(() => localStorage.getItem('OrbitOS-live-wall') || 'live-1')
  const [widgets, setWidgets] = useState(() => localStorage.getItem('OrbitOS-widgets') !== 'off')

  useEffect(() => {
    localStorage.setItem('OrbitOS-accent', accent)
  ...
  useEffect(() => {
    localStorage.setItem('OrbitOS-bg', bgMode)
    window.dispatchEvent(new CustomEvent('OrbitOS-bg-change', { detail: { mode: bgMode, wallpaper: selectedWallpaper, liveWall: selectedLiveWall } }))
  }, [bgMode, selectedWallpaper, selectedLiveWall])

  useEffect(() => { localStorage.setItem('OrbitOS-wallpaper', selectedWallpaper) }, [selectedWallpaper])
  useEffect(() => { localStorage.setItem('OrbitOS-live-wall', selectedLiveWall) }, [selectedLiveWall])
  useEffect(() => { localStorage.setItem('OrbitOS-widgets', widgets ? 'on' : 'off') }, [widgets])
  ...
  return (
    <div style={{ padding: 20, overflow: 'auto', height: '100%' }}>
      ...
      <Section title="Orbit Assistant">
        <div style={{ marginBottom: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-text-primary)' }}>Gemini API Key</span>
          <div style={{ fontSize: 10, color: 'var(--color-text-muted)', marginTop: 2 }}>Required for Orbit AI responses. Get one at aistudio.google.com</div>
        </div>
        <input
          type="password"
          defaultValue={localStorage.getItem('OrbitOS-gemini-key') || 'AIzaSyDxvt3A5saNeoG9FeAFqNcQmL-wmqXokJ0'}
          onBlur={(e) => localStorage.setItem('OrbitOS-gemini-key', e.target.value)}
        ...
        <div style={{ marginTop: 6, fontSize: 11, color: 'var(--color-text-muted)' }}>
          Ctrl+M to toggle Orbit · Click the orb to talk
        </div>
      </Section>

      <Section title="About">
        <InfoRow label="Version" value="OrbitOS 1.0.0" />
        <InfoRow label="Made by" value="Hack Clubber" />
      </Section>
