...
const COMMANDS: Record<string, (text?: string) => string> = {
  ...
  whoami: () => 'orbit-user -- commander of the ISS, explorer of the digital cosmos',

  about: () => `OrbitOS v1.0.0
A space-themed web operating system built for the Hack Club.
Made with React, TypeScript, Tailwind CSS, and starry vibes.`,

  skills: () => `Languages:    JavaScript, TypeScript, HTML/CSS
Frameworks:  React, Vite
Theme:       Space, Orbit, ISS`,

  projects: () => `OrbitOS      A next-generation web operating system
ISS Tracker  Real-time satellite tracking`,

  contact: () => `GitHub:   github.com/hackclub
 ...
 *   .   *      Kernel: React 19 + Vite
  ...
  weather: () => {
    ...
    return `OrbitOS Weather Service\n${cond} | ${temp}°C\nHumidity: ${Math.floor(Math.random() * 60 + 30)}%\nWind: ${Math.floor(Math.random() * 20 + 1)} km/h\nVibes: Immaculate ✨`
  },

  cowsay: (text?: string) => {
    const msg = text || 'OrbitOS is kawaii!'
...
export default function Terminal() {
  const [lines, setLines] = useState<Line[]>([
    { text: 'Welcome to OrbitOSTerm v1.0.0', type: 'output' },
  ...
  const handleCommand = (cmd: string) => {
    ...
    if (command === 'echo') {
      ...
      newLines.push({ text: `OrbitOSsh: command not found: ${command}`, type: 'error' })
    }
