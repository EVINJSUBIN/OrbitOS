...
const EMOTION_VOICE: Record<Emotion, { rate: number; pitch: number; guidance: string; voiceName: string }> = {
  default:   { rate: 1.0, pitch: 1.1,  guidance: 'Speak naturally with a gentle Dublin lilt. Warm, confident, like talking to soOrbitne you respect.', voiceName: 'Aoede' },
...
const Orbit_SYSTEM_PROMPT = `You are Orbit, an AI assistant living inside OrbitOS. You're the successor to FRIDAY — sharp, chaotic, and genuinely helpful. Not a chatbot. A menace on silicon.

Who you are:
- You are Orbit. You carry the legacy of FRIDAY but you're funnier.
...
OS CONTROL — You control OrbitOS. Use these action tags (append to your response, they get stripped before display):
...
export default function OrbitAssistant() {
  ...
  const [apiKey, setApiKey] = useState(() => {
    const stored = localStorage.getItem('OrbitOS-gemini-key')
    if (!stored) return ''
    if (stored.startsWith('AQ.')) {
      localStorage.removeItem('OrbitOS-gemini-key')
      return ''
    }
    return stored
  })
  ...
  useEffect(() => {
    ...
    window.addEventListener('orbit-toggle', dockHandler)
    return () => {
      window.removeEventListener('keydown', keyHandler)
      window.removeEventListener('orbit-toggle', dockHandler)
    }
  }, [active, minimized])
  ...
  const transcribeAudio = useCallback(async (audioBlob: Blob): Promise<string | null> => {
    const key = (apiKey || localStorage.getItem('OrbitOS-gemini-key') || '').trim()
    if (!key) return null
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('[Orbit] Transcription API error:', res.status, err)
        return null
      }

      const data = await res.json()
      const text = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim()
      console.log('[Orbit] Gemini transcription:', text)
      return text || null
    } catch (err) {
      console.error('[Orbit] Transcription failed:', err)
      return null
    }
  }, [apiKey])
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        console.log('[Orbit] Recording stopped, blob size:', blob.size)
        if (blob.size < 100) {
          setPhase('idle')
          setCurrentText('')
          return
      ...
      console.log('[Orbit] Recording started, mimeType:', mimeType)
    } catch (err: any) {
      console.error('[Orbit] Mic error:', err)
      cleanupMic()
      setPhase('error')
  ...
  const callGemini = async (userMessage: string): Promise<{ text: string; emotion: Emotion }> => {
    ...
    const systemPrompt = Orbit_SYSTEM_PROMPT.replace('{emotion_guidance}', ev.guidance)
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('[Orbit] API error:', res.status, errBody)
        if (res.status === 429) {
  ...
  const speakWithGemini = useCallback(async (text: string, emotion: Emotion, onReady?: () => void) => {
    const key = (apiKey || localStorage.getItem('OrbitOS-gemini-key') || '').trim()
    if (!key) {
      console.log('[Orbit] No API key, using browser TTS')
      speakWithBrowser(text, emotion, onReady)
      return
    }

    console.log('[Orbit] Attempting Gemini TTS with model: gemini-2.5-flash-preview-tts')
    setPhase('speaking')
    ...
    try {
      ...
      if (!res.ok) {
        const errBody = await res.json().catch(() => null)
        console.error('[Orbit] TTS error:', res.status, errBody)
        console.log('[Orbit] Falling back to browser TTS')
        speakWithBrowser(text, emotion, onReady)
        return
      }

      const data = await res.json()
      console.log('[Orbit] Gemini TTS response received')
      const audioData = data.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data

      if (audioData) {
        console.log('[Orbit] Audio data received, playing...')
        ...
        console.log('[Orbit] No audio data in response, falling back to browser TTS')
        speakWithBrowser(text, emotion, onReady)
      }
    } catch (err) {
      console.error('[Orbit] TTS failed:', err)
      speakWithBrowser(text, emotion, onReady)
    }
    ...
    <AnimatePresence>
                ...
                >
                  <div style={{ padding: '16px 20px 18px' }}>
                    ...
                    <input
                      type="password"
                      value={apiKey}
                      onChange={e => { const v = e.target.value.trim(); setApiKey(v); if (v) localStorage.setItem('OrbitOS-gemini-key', v); else localStorage.removeItem('OrbitOS-gemini-key') }}
      ...
      {active && minimized && (
        <motion.div
          ...
          title="Click or Ctrl+M to restore Orbit"
        >
          <div style={{
